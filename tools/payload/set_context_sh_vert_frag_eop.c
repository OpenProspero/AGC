 ...
 * patched to uploaded vert code) with the Step T smoke.frag register
 * program (context + SH, PGM patched to uploaded frag code), in host
...
static const char openagc_log_path[] =
    "/data/prosperoai/openagc-set-context-sh-vert-frag-eop.log";
...
int main(void)
{
    ...
    if (sceKernelAllocateMainDirectMemory(OPENAGC_ARENA, OPENAGC_ARENA, 1,
                                          &physical) != 0) {
        return openagc_logf("openagc-set-context-sh-vert-frag-eop: allocate failed\n") ==
                       0
                   ? 0
                   : 1;
    }
    if (sceKernelMapNamedDirectMemory((void **)&arena, OPENAGC_ARENA,
                                      ...
                                      OPENAGC_ARENA, "openagc-vert-frag") != 0) {
        return openagc_logf("openagc-set-context-sh-vert-frag-eop: map failed\n") == 0
                   ? 0
                   : 1;
    }
    ...
    if ((vert_code_va & 0xffu) != 0u || (frag_code_va & 0xffu) != 0u) {
        return openagc_logf(
                   "openagc-set-context-sh-vert-frag-eop: code_va misaligned\n") == 0
                   ? 0
                   : 1;
    }
    ...
    *marker = 0u;
    ...
    if (word_count != OPENAGC_STEP_U_WORDS) {
        return openagc_logf(
                   "openagc-set-context-sh-vert-frag-eop: encode size mismatch\n") == 0
                   ? 0
                   : 1;
    }
    ...
    if (gc_fd < 0) {
        return openagc_logf(
                   "openagc-set-context-sh-vert-frag-eop: /dev/gc unavailable\n") == 0
                   ? 0
                   : 1;
    }
    if (ioctl(gc_fd, OPENAGC_CONTEXT_QUERY, &submit) != 0) {
        close(gc_fd);
        return openagc_logf(
                   "openagc-set-context-sh-vert-frag-eop: context query refused\n") ==
                       0
                   ? 0
                   : 1;
    }
    ...
    if (ioctl(gc_fd, OPENAGC_SUBMIT_16, &submit) != 0) {
        close(gc_fd);
        return openagc_logf("openagc-set-context-sh-vert-frag-eop: submit refused\n") ==
                       0
                   ? 0
                   : 1;
    }
    ...
    (void)openagc_logf(
        "openagc-set-context-sh-vert-frag-eop: submit=ok completed=%d "
        "v_ctx=%u v_sh=%u link=3 f_ctx=%u f_sh=%u words=%u "
        "vert_va=%016llx frag_va=%016llx marker=%llx\n",
        completed, OPENAGC_VERT_CTX_COUNT, OPENAGC_VERT_SH_COUNT,
        OPENAGC_FRAG_CTX_COUNT, OPENAGC_FRAG_SH_COUNT, OPENAGC_STEP_U_WORDS,
        (unsigned long long)vert_code_va, (unsigned long long)frag_code_va,
