 ...
 * shader_registers pairs (PGM patched to uploaded smoke.frag code) plus
...
#define OPENAGC_CTX_PAIR_COUNT 9u
#define OPENAGC_SH_PAIR_COUNT 4u
#define OPENAGC_STEP_T_WORDS \
    OPENAGC_PM4_GRAPHICS_CONTEXT_SH_EOP_WORDS(OPENAGC_CTX_PAIR_COUNT, \
                                              OPENAGC_SH_PAIR_COUNT)
...
/* smoke.frag.metadata.json shader_registers offsets; PGM values patched at runtime. */
static const uint32_t openagc_sh_offsets[OPENAGC_SH_PAIR_COUNT] = {
    8u, 9u, 10u, 11u
};
static const uint32_t openagc_sh_rsrc1 = 36438017u;
static const uint32_t openagc_sh_rsrc2 = 4u;

static const char openagc_log_path[] =
    "/data/prosperoai/openagc-set-context-sh-frag-eop.log";
...
int main(void)
{
    ...
    uint32_t words[OPENAGC_STEP_T_WORDS];
    uint32_t sh_values[OPENAGC_SH_PAIR_COUNT];
    uint32_t word_count;
    int gc_fd;
    int completed = 0;
    off_t physical = 0;
    ...
    if (sceKernelAllocateMainDirectMemory(OPENAGC_ARENA, OPENAGC_ARENA, 1,
                                          &physical) != 0) {
        return openagc_logf("openagc-set-context-sh-frag-eop: allocate failed\n") == 0
                   ? 0
                   : 1;
    }
    if (sceKernelMapNamedDirectMemory((void **)&arena, OPENAGC_ARENA,
                                      ...
                                      OPENAGC_ARENA, "openagc-ctx-sh-frag") != 0) {
        return openagc_logf("openagc-set-context-sh-frag-eop: map failed\n") == 0 ? 0
                                                                                   : 1;
    }
    ...
    if ((code_va & 0xffu) != 0u) {
        return openagc_logf("openagc-set-context-sh-frag-eop: code_va misaligned\n") ==
                       0
                   ? 0
                   : 1;
    }
    ...
    memcpy(code, openagc_smoke_frag_code, sizeof(openagc_smoke_frag_code));
    *marker = 0u;

    sh_values[0] = (uint32_t)(code_va >> 8);
    sh_values[1] = (uint32_t)(code_va >> 40);
    sh_values[2] = openagc_sh_rsrc1;
    sh_values[3] = openagc_sh_rsrc2;
    word_count = openagc_pm4_encode_graphics_context_sh_eop(
        openagc_ctx_offsets, openagc_ctx_values, OPENAGC_CTX_PAIR_COUNT,
        openagc_sh_offsets, sh_values, OPENAGC_SH_PAIR_COUNT, OPENAGC_EOP_SEQUENCE,
        marker_va, words);
    if (word_count != OPENAGC_STEP_T_WORDS) {
        return openagc_logf(
                   "openagc-set-context-sh-frag-eop: encode size mismatch\n") == 0
                   ? 0
                   : 1;
    }
    ...
    cb[0].ib_base = ((uint64_t)OPENAGC_STEP_T_WORDS << 32) | ((uint64_t)ib_va >> 32);

    gc_fd = open("/dev/gc", O_RDWR);
    if (gc_fd < 0) {
        return openagc_logf("openagc-set-context-sh-frag-eop: /dev/gc unavailable\n") ==
                       0
                   ? 0
                   : 1;
    }
    if (ioctl(gc_fd, OPENAGC_CONTEXT_QUERY, &submit) != 0) {
        close(gc_fd);
        return openagc_logf(
                   "openagc-set-context-sh-frag-eop: context query refused\n") == 0
                   ? 0
                   : 1;
    }
    ...
    if (ioctl(gc_fd, OPENAGC_SUBMIT_16, &submit) != 0) {
        close(gc_fd);
        return openagc_logf("openagc-set-context-sh-frag-eop: submit refused\n") == 0
                   ? 0
                   : 1;
    }
    ...
    (void)openagc_logf("openagc-set-context-sh-frag-eop: submit=ok completed=%d "
                       "ctx=%u sh=%u words=%u code_va=%016llx marker=%llx\n",
                       completed, OPENAGC_CTX_PAIR_COUNT, OPENAGC_SH_PAIR_COUNT,
                       OPENAGC_STEP_T_WORDS, (unsigned long long)code_va,
