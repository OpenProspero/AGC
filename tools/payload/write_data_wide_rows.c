...
static const char openagc_log_path[] =
    "/data/prosperoai/openagc-write-data-wide-rows.log";
...
int main(void)
{
    ...
    uint32_t words[OPENAGC_PM4_WRITE_DATA_STEP_G_EOP_WORDS];
    ...
    if (sceKernelAllocateMainDirectMemory(OPENAGC_ARENA, OPENAGC_ARENA, 1,
                                          &physical) != 0) {
        return openagc_logf("openagc-write-data-wide-rows: allocate failed\n") == 0
                   ? 0
                   : 1;
    }
    if (sceKernelMapNamedDirectMemory((void **)&arena, OPENAGC_ARENA,
                                      ...
                                      OPENAGC_ARENA, "openagc-wdwr") != 0) {
        return openagc_logf("openagc-write-data-wide-rows: map failed\n") == 0 ? 0
                                                                              : 1;
    }
    ...
    *marker = 0u;
    ...
    openagc_pm4_encode_write_data_rows_eop(destination_va, OPENAGC_ROW_PITCH,
                                           ...
                                           OPENAGC_PM4_WRITE_DATA_STEP_G_ROWS,
                                           OPENAGC_EOP_SEQUENCE, marker_va, words);
    memcpy(ib, words, sizeof(words));

    cb[0].header = ((uint64_t)ib_va << 32) | 0xC0023F00u;
    cb[0].ib_base = ((uint64_t)OPENAGC_PM4_WRITE_DATA_STEP_G_EOP_WORDS << 32) |
                    ((uint64_t)ib_va >> 32);

    gc_fd = open("/dev/gc", O_RDWR);
    if (gc_fd < 0) {
        return openagc_logf("openagc-write-data-wide-rows: /dev/gc unavailable\n") ==
                       0
                   ? 0
                   : 1;
    }
    if (ioctl(gc_fd, OPENAGC_CONTEXT_QUERY, &submit) != 0) {
        close(gc_fd);
        return openagc_logf(
                   "openagc-write-data-wide-rows: context query refused\n") == 0
                   ? 0
                   : 1;
    }
    ...
    if (ioctl(gc_fd, OPENAGC_SUBMIT_16, &submit) != 0) {
        close(gc_fd);
        return openagc_logf("openagc-write-data-wide-rows: submit refused\n") == 0
                   ? 0
                   : 1;
    }
    ...
    for (row = 0u; row < OPENAGC_PM4_WRITE_DATA_STEP_G_ROWS; ++row) {
        uint32_t *row_words =
            (uint32_t *)(destination + (size_t)row * OPENAGC_ROW_PITCH);

        for (col = 0u; col < OPENAGC_PM4_WRITE_DATA_STEP_G_ROW_DWORDS; ++col) {
            if (row_words[col] != OPENAGC_WRITE_VALUE) {
                matched = 0;
            }
        }
        /* Gap between filled width and pitch must stay 0xCC. */
        if (destination[(size_t)row * OPENAGC_ROW_PITCH +
                        OPENAGC_PM4_WRITE_DATA_STEP_G_ROW_DWORDS * 4u] != 0xCCu) {
            matched = 0;
        }
    }
    ...
    (void)openagc_logf(
        "openagc-write-data-wide-rows: submit=ok completed=%d matched=%d "
        "destination=%08x%08x rows=%u dwords=%u pitch=%u value=%08x "
        "marker=%llx\n",
        completed, matched, dest_hi, dest_lo, OPENAGC_PM4_WRITE_DATA_STEP_G_ROWS,
        OPENAGC_PM4_WRITE_DATA_STEP_G_ROW_DWORDS, OPENAGC_ROW_PITCH,
