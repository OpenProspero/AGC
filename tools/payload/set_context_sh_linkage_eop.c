 ...
 * patched to uploaded code (Step O), and linkage SET_CONTEXT pairs
...
#define OPENAGC_STEP_S_WORDS \
    OPENAGC_PM4_GRAPHICS_CONTEXT_SH_LINKAGE_EOP_WORDS(OPENAGC_CTX_PAIR_COUNT, \
...
/* smoke.vert.metadata.json linkage — verified offsets/values only. */
...
static const char openagc_log_path[] =
    "/data/prosperoai/openagc-set-context-sh-linkage-eop.log";
...
int main(void)
{
    ...
    uint32_t words[OPENAGC_STEP_S_WORDS];
    ...
    if (sceKernelAllocateMainDirectMemory(OPENAGC_ARENA, OPENAGC_ARENA, 1,
                                          &physical) != 0) {
        return openagc_logf("openagc-set-context-sh-linkage-eop: allocate failed\n") == 0
                   ? 0
                   : 1;
    }
    if (sceKernelMapNamedDirectMemory((void **)&arena, OPENAGC_ARENA,
                                      ...
                                      OPENAGC_ARENA, "openagc-ctx-sh-ln") != 0) {
        return openagc_logf("openagc-set-context-sh-linkage-eop: map failed\n") == 0
                   ? 0
                   : 1;
    }
    ...
    if ((code_va & 0xffu) != 0u) {
        return openagc_logf("openagc-set-context-sh-linkage-eop: code_va misaligned\n") ==
                       0
                   ? 0
                   : 1;
    }
    ...
    *marker = 0u;
    ...
    word_count = openagc_pm4_encode_graphics_context_sh_linkage_eop(
        openagc_ctx_offsets, openagc_ctx_values, OPENAGC_CTX_PAIR_COUNT,
        openagc_sh_offsets, sh_values, OPENAGC_SH_PAIR_COUNT, OPENAGC_LINK_GE_CNTL_OFF,
        OPENAGC_LINK_GE_CNTL_VAL, OPENAGC_LINK_STAGES_EN_OFF, OPENAGC_LINK_STAGES_EN_VAL,
        OPENAGC_LINK_USER_VGPR_EN_OFF, OPENAGC_LINK_USER_VGPR_EN_VAL, OPENAGC_EOP_SEQUENCE,
        marker_va, words);
    if (word_count != OPENAGC_STEP_S_WORDS) {
        return openagc_logf("openagc-set-context-sh-linkage-eop: encode size mismatch\n") ==
                       0
                   ? 0
                   : 1;
    }
    ...
    cb[0].ib_base = ((uint64_t)OPENAGC_STEP_S_WORDS << 32) | ((uint64_t)ib_va >> 32);

    gc_fd = open("/dev/gc", O_RDWR);
    if (gc_fd < 0) {
        return openagc_logf(
                   "openagc-set-context-sh-linkage-eop: /dev/gc unavailable\n") == 0
                   ? 0
                   : 1;
    }
    if (ioctl(gc_fd, OPENAGC_CONTEXT_QUERY, &submit) != 0) {
        close(gc_fd);
        return openagc_logf(
                   "openagc-set-context-sh-linkage-eop: context query refused\n") == 0
                   ? 0
                   : 1;
    }
    ...
    if (ioctl(gc_fd, OPENAGC_SUBMIT_16, &submit) != 0) {
        close(gc_fd);
        return openagc_logf("openagc-set-context-sh-linkage-eop: submit refused\n") == 0
                   ? 0
                   : 1;
    }
    ...
    (void)openagc_logf("openagc-set-context-sh-linkage-eop: submit=ok completed=%d "
                       "ctx=%u sh=%u link=3 words=%u code_va=%016llx marker=%llx\n",
                       completed, OPENAGC_CTX_PAIR_COUNT, OPENAGC_SH_PAIR_COUNT,
                       OPENAGC_STEP_S_WORDS, (unsigned long long)code_va,
