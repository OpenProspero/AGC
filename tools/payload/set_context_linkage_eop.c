...
#define OPENAGC_STEP_R_WORDS OPENAGC_PM4_GRAPHICS_LINKAGE_EOP_WORDS
...
static const char openagc_log_path[] =
    "/data/prosperoai/openagc-set-context-linkage-eop.log";
...
int main(void)
{
    ...
    uint32_t words[OPENAGC_STEP_R_WORDS];
    uint32_t word_count;
    int gc_fd;
    int completed = 0;
    off_t physical = 0;
    ...
    if (sceKernelAllocateMainDirectMemory(OPENAGC_ARENA, OPENAGC_ARENA, 1,
                                          &physical) != 0) {
        return openagc_logf("openagc-set-context-linkage-eop: allocate failed\n") == 0
                   ? 0
                   : 1;
    }
    if (sceKernelMapNamedDirectMemory((void **)&arena, OPENAGC_ARENA,
                                      ...
                                      OPENAGC_ARENA, "openagc-set-link") != 0) {
        return openagc_logf("openagc-set-context-linkage-eop: map failed\n") == 0 ? 0
                                                                                   : 1;
    }
    ...
    *marker = 0u;
    ...
    word_count = openagc_pm4_encode_graphics_linkage_eop(
        ...
        OPENAGC_LINK_USER_VGPR_EN_VAL, OPENAGC_EOP_SEQUENCE, marker_va, words);
    if (word_count != OPENAGC_STEP_R_WORDS) {
        return openagc_logf("openagc-set-context-linkage-eop: encode size mismatch\n") ==
                       0
                   ? 0
                   : 1;
    }
    ...
    cb[0].ib_base = ((uint64_t)OPENAGC_STEP_R_WORDS << 32) | ((uint64_t)ib_va >> 32);

    gc_fd = open("/dev/gc", O_RDWR);
    if (gc_fd < 0) {
        return openagc_logf("openagc-set-context-linkage-eop: /dev/gc unavailable\n") ==
                       0
                   ? 0
                   : 1;
    }
    if (ioctl(gc_fd, OPENAGC_CONTEXT_QUERY, &submit) != 0) {
        close(gc_fd);
        return openagc_logf(
                   "openagc-set-context-linkage-eop: context query refused\n") == 0
                   ? 0
                   : 1;
    }
    ...
    if (ioctl(gc_fd, OPENAGC_SUBMIT_16, &submit) != 0) {
        close(gc_fd);
        return openagc_logf("openagc-set-context-linkage-eop: submit refused\n") == 0
                   ? 0
                   : 1;
    }
    ...
    (void)openagc_logf("openagc-set-context-linkage-eop: submit=ok completed=%d "
                       "pairs=3 words=%u marker=%llx\n",
                       completed, OPENAGC_STEP_R_WORDS, (unsigned long long)*marker);
