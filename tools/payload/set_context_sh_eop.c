 ...
 * graphics-bank shader_registers pairs (Step O, PGM patched to uploaded
...
/* smoke.vert.metadata.json — context_registers only (no linkage). */
...
static const char openagc_log_path[] = "/data/prosperoai/openagc-set-context-sh-eop.log";
...
int main(void)
{
    ...
    uint32_t words[OPENAGC_STEP_Q_WORDS];
    ...
    if (sceKernelAllocateMainDirectMemory(OPENAGC_ARENA, OPENAGC_ARENA, 1,
                                          &physical) != 0) {
        return openagc_logf("openagc-set-context-sh-eop: allocate failed\n") == 0 ? 0
                                                                                    : 1;
    }
    if (sceKernelMapNamedDirectMemory((void **)&arena, OPENAGC_ARENA,
                                      ...
                                      OPENAGC_ARENA, "openagc-ctx-sh") != 0) {
        return openagc_logf("openagc-set-context-sh-eop: map failed\n") == 0 ? 0 : 1;
    }
    ...
    if ((code_va & 0xffu) != 0u) {
        return openagc_logf("openagc-set-context-sh-eop: code_va misaligned\n") == 0
                   ? 0
                   : 1;
    }
    ...
    *marker = 0u;
    ...
    word_count = openagc_pm4_encode_graphics_context_sh_eop(
        ...
        marker_va, words);
    if (word_count != OPENAGC_STEP_Q_WORDS) {
        return openagc_logf("openagc-set-context-sh-eop: encode size mismatch\n") == 0
                   ? 0
                   : 1;
    }
    ...
    cb[0].ib_base = ((uint64_t)OPENAGC_STEP_Q_WORDS << 32) | ((uint64_t)ib_va >> 32);

    gc_fd = open("/dev/gc", O_RDWR);
    if (gc_fd < 0) {
        return openagc_logf("openagc-set-context-sh-eop: /dev/gc unavailable\n") == 0
                   ? 0
                   : 1;
    }
    if (ioctl(gc_fd, OPENAGC_CONTEXT_QUERY, &submit) != 0) {
        close(gc_fd);
        return openagc_logf("openagc-set-context-sh-eop: context query refused\n") == 0
                   ? 0
                   : 1;
    }
    ...
    if (ioctl(gc_fd, OPENAGC_SUBMIT_16, &submit) != 0) {
        close(gc_fd);
        return openagc_logf("openagc-set-context-sh-eop: submit refused\n") == 0 ? 0
                                                                                  : 1;
    }
    ...
    (void)openagc_logf("openagc-set-context-sh-eop: submit=ok completed=%d "
                       "ctx=%u sh=%u words=%u code_va=%016llx marker=%llx\n",
                       completed, OPENAGC_CTX_PAIR_COUNT, OPENAGC_SH_PAIR_COUNT,
                       OPENAGC_STEP_Q_WORDS, (unsigned long long)code_va,
