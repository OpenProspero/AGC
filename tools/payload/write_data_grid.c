...
#define OPENAGC_ROW_PITCH \
    (OPENAGC_PM4_WRITE_DATA_STEP_N_COLS * OPENAGC_PM4_WRITE_DATA_STEP_N_COL_DWORDS * 4u)
#define OPENAGC_EOP_SEQUENCE 1u
#define OPENAGC_DEADLINE_SECONDS 30
#define OPENAGC_FILL_BYTES \
    (OPENAGC_PM4_WRITE_DATA_STEP_N_ROWS * OPENAGC_ROW_PITCH)
...
static const char openagc_log_path[] =
    "/data/prosperoai/openagc-write-data-grid.log";
...
int main(void)
{
    ...
    uint32_t words[OPENAGC_PM4_WRITE_DATA_STEP_N_EOP_WORDS];
    ...
    if (sceKernelAllocateMainDirectMemory(OPENAGC_ARENA, OPENAGC_ARENA, 1,
                                          &physical) != 0) {
        return openagc_logf("openagc-write-data-grid: allocate failed\n") == 0 ? 0 : 1;
    }
    if (sceKernelMapNamedDirectMemory((void **)&arena, OPENAGC_ARENA,
                                      ...
                                      OPENAGC_ARENA, "openagc-wdg") != 0) {
        return openagc_logf("openagc-write-data-grid: map failed\n") == 0 ? 0 : 1;
    }
    ...
    *marker = 0u;
    ...
    openagc_pm4_encode_write_data_grid_eop(
        destination_va, OPENAGC_ROW_PITCH, OPENAGC_WRITE_VALUE,
        OPENAGC_PM4_WRITE_DATA_STEP_N_COL_DWORDS, OPENAGC_PM4_WRITE_DATA_STEP_N_COLS,
        OPENAGC_PM4_WRITE_DATA_STEP_N_ROWS, OPENAGC_EOP_SEQUENCE, marker_va, words);
    memcpy(ib, words, sizeof(words));

    cb[0].header = ((uint64_t)ib_va << 32) | 0xC0023F00u;
    cb[0].ib_base = ((uint64_t)OPENAGC_PM4_WRITE_DATA_STEP_N_EOP_WORDS << 32) |
                    ((uint64_t)ib_va >> 32);

    gc_fd = open("/dev/gc", O_RDWR);
    if (gc_fd < 0) {
        return openagc_logf("openagc-write-data-grid: /dev/gc unavailable\n") == 0 ? 0
                                                                                    : 1;
    }
    if (ioctl(gc_fd, OPENAGC_CONTEXT_QUERY, &submit) != 0) {
        close(gc_fd);
        return openagc_logf("openagc-write-data-grid: context query refused\n") == 0
                   ? 0
                   : 1;
    }
    ...
    if (ioctl(gc_fd, OPENAGC_SUBMIT_16, &submit) != 0) {
        close(gc_fd);
        return openagc_logf("openagc-write-data-grid: submit refused\n") == 0 ? 0 : 1;
    }
    ...
    for (i = 0u; i < OPENAGC_FILL_BYTES / 4u; ++i) {
        if (((uint32_t *)destination)[i] != OPENAGC_WRITE_VALUE) {
            matched = 0;
        }
    }
    if (destination[OPENAGC_FILL_BYTES] != 0xCCu) {
        matched = 0;
    }
    dest_lo = (uint32_t)destination_va;
    dest_hi = (uint32_t)(destination_va >> 32);
    (void)openagc_logf(
        "openagc-write-data-grid: submit=ok completed=%d matched=%d "
        "destination=%08x%08x rows=%u cols=%u dwords=%u pitch=%u value=%08x "
        "marker=%llx\n",
        completed, matched, dest_hi, dest_lo, OPENAGC_PM4_WRITE_DATA_STEP_N_ROWS,
        OPENAGC_PM4_WRITE_DATA_STEP_N_COLS, OPENAGC_PM4_WRITE_DATA_STEP_N_COL_DWORDS,
        OPENAGC_ROW_PITCH, *(uint32_t *)destination, (unsigned long long)*marker);
