...
/* Step X: COPY_DATA CB probe with absolute CONTEXT_REG_START+offset src + EOP.
 *
 * Distinct from Step W (relative SET_CONTEXT offsets as src_lo, console-
 * negative). Hypothesis: gfx_v10 emit_rreg passes absolute mm-mapped
 * register addresses; for context regs that is PACKET3_SET_CONTEXT_REG_START
 * (0xA000) + relative offset (soc15d.h). Same control word as Step W.
 *
...
static const char openagc_log_path[] =
    "/data/prosperoai/openagc-ib-dump-ctxreg-abs.log";
...
int main(void)
{
    ...
    if (sceKernelAllocateMainDirectMemory(OPENAGC_ARENA, OPENAGC_ARENA, 1,
                                          &physical) != 0) {
        return openagc_logf("openagc-ctxreg-abs: allocate failed\n") == 0 ? 0 : 1;
    }
    if (sceKernelMapNamedDirectMemory((void **)&arena, OPENAGC_ARENA,
                                      ...
                                      OPENAGC_ARENA, "openagc-ctxreg-abs") != 0) {
        return openagc_logf("openagc-ctxreg-abs: map failed\n") == 0 ? 0 : 1;
    }
    ...
    *marker = 0u;
    ...
    word_count = openagc_pm4_encode_copy_data_cb_probe_abs_eop(
        dest_va, OPENAGC_EOP_SEQUENCE, marker_va, words);
    if (word_count != OPENAGC_PM4_COPY_DATA_CB_PROBE_EOP_WORDS) {
        return openagc_logf("openagc-ctxreg-abs: encode size mismatch\n") == 0 ? 0
                                                                                : 1;
    }
    ...
    if (gc_fd < 0) {
        return openagc_logf("openagc-ctxreg-abs: /dev/gc unavailable\n") == 0 ? 0
                                                                               : 1;
    }
    if (ioctl(gc_fd, OPENAGC_CONTEXT_QUERY, &submit) != 0) {
        close(gc_fd);
        return openagc_logf("openagc-ctxreg-abs: context query refused\n") == 0
                   ? 0
                   : 1;
    }
    ...
    if (ioctl(gc_fd, OPENAGC_SUBMIT_16, &submit) != 0) {
        close(gc_fd);
        return openagc_logf("openagc-ctxreg-abs: submit refused\n") == 0 ? 0 : 1;
    }
