...
/* Step H: DMA + WRITE_DATA + EOP in one IB for FW9.40.
 *
 * Contract: one submit, one IT_DMA_DATA (64 bytes), one WRITE_DATA
 * (4 dwords over the destination start), one shared EOP+NOP trailer,
 * finite deadline, no shader, no CB/DB, no draw, no retry. One log line.
 *
 * Layout: include/openagc/pm4_write_fw940.h
 * (Steps B and D–G proved DMA and WRITE_DATA alone; this proves both
 *  in one graphics-queue IB for copy-then-clear host recording.)
 */
...
#define OPENAGC_DMA_VALUE 0x11111111u
...
static const char openagc_log_path[] = "/data/prosperoai/openagc-dma-write-eop.log";
...
int main(void)
{
    ...
    uint32_t words[OPENAGC_PM4_DMA_WRITE_STEP_H_EOP_WORDS];
    uint32_t i;
    int gc_fd;
    int completed = 0;
    int matched = 1;
    ...
    if (sceKernelAllocateMainDirectMemory(OPENAGC_ARENA, OPENAGC_ARENA, 1,
                                          &physical) != 0) {
        return openagc_logf("openagc-dma-write-eop: allocate failed\n") == 0 ? 0 : 1;
    }
    if (sceKernelMapNamedDirectMemory((void **)&arena, OPENAGC_ARENA,
                                      ...
                                      OPENAGC_ARENA, "openagc-dwe") != 0) {
        return openagc_logf("openagc-dma-write-eop: map failed\n") == 0 ? 0 : 1;
    }

    source = (uint32_t *)(arena + OPENAGC_SOURCE_OFF);
    ...
    *marker = 0u;
    for (i = 0u; i < OPENAGC_COPY_BYTES / 4u; ++i) {
        source[i] = OPENAGC_DMA_VALUE;
    }

    openagc_pm4_encode_dma_write_data_eop(
        source_va, destination_va, OPENAGC_COPY_BYTES, OPENAGC_WRITE_VALUE,
        OPENAGC_PM4_WRITE_DATA_STEP_H_DWORDS, OPENAGC_EOP_SEQUENCE, marker_va, words);
    memcpy(ib, words, sizeof(words));

    cb[0].header = ((uint64_t)ib_va << 32) | 0xC0023F00u;
    cb[0].ib_base = ((uint64_t)OPENAGC_PM4_DMA_WRITE_STEP_H_EOP_WORDS << 32) |
                    ((uint64_t)ib_va >> 32);

    gc_fd = open("/dev/gc", O_RDWR);
    if (gc_fd < 0) {
        return openagc_logf("openagc-dma-write-eop: /dev/gc unavailable\n") == 0 ? 0 : 1;
    }
    if (ioctl(gc_fd, OPENAGC_CONTEXT_QUERY, &submit) != 0) {
        close(gc_fd);
        return openagc_logf("openagc-dma-write-eop: context query refused\n") == 0 ? 0 : 1;
    }
    ...
    if (ioctl(gc_fd, OPENAGC_SUBMIT_16, &submit) != 0) {
        close(gc_fd);
        return openagc_logf("openagc-dma-write-eop: submit refused\n") == 0 ? 0 : 1;
    }
    ...
    for (i = 0u; i < OPENAGC_PM4_WRITE_DATA_STEP_H_DWORDS; ++i) {
        if (destination[i] != OPENAGC_WRITE_VALUE) {
            matched = 0;
        }
    }
    for (i = OPENAGC_PM4_WRITE_DATA_STEP_H_DWORDS; i < OPENAGC_COPY_BYTES / 4u; ++i) {
        if (destination[i] != OPENAGC_DMA_VALUE) {
            matched = 0;
        }
    }
    if (arena[OPENAGC_DEST_OFF + OPENAGC_COPY_BYTES] != 0xCCu) {
        matched = 0;
    }

    (void)openagc_logf(
        "openagc-dma-write-eop: submit=ok completed=%d matched=%d "
        "dma_bytes=%u write_dwords=%u dma_tail=%08x write_head=%08x marker=%llx\n",
        completed, matched, OPENAGC_COPY_BYTES, OPENAGC_PM4_WRITE_DATA_STEP_H_DWORDS,
        destination[OPENAGC_PM4_WRITE_DATA_STEP_H_DWORDS], destination[0],
