...
/* Step AD: the NGG draw - one point through the compiler's NGG vertex stage.
 *
 * Step AC proved the whole submission path but produced no fragments: this
 * firmware keeps the uconfig aperture read-only, so VGT_PRIMITIVE_TYPE never
 * leaves DI_PT_NONE and the legacy VGT assembles nothing. For NGG the input
 * topology is a context register (VGT_GS_OUT_PRIM_TYPE), and the vertex
 * stage is the ES block the console's own driver uses, so this payload runs
 ...
 * vec4(1.0, 0.25, 0.0, 1.0), so acceptance is exactly Step AC's: one 8x8
 * point inside the 32x32 linear RGBA8 target, every written dword
 * 0xFF0040FF, nothing written outside the rect, and the NGG readbacks
 * (VGT_SHADER_STAGES_EN, GE_CNTL, ES PGM LO, the LDS layout dword) showing
...
/* The context register table the NGG program is loaded through. */
#define OPENAGC_TABLE_OFF 0x1300u
...
static const char openagc_log_path[] =
    "/data/prosperoai/openagc-ib-dump-draw-ngg.log";
                                   ...
                                   const uint32_t *ngg, const uint32_t *window)
{
    ...
    n = snprintf(buffer, sizeof(buffer),
                 "openagc-draw-ngg-owned: color_va=%016llx rect=%u,%u,%ux%u "
    ...
    /* In this order: VGT_SHADER_STAGES_EN (PRIMGEN_EN set), GE_CNTL,
     * SPI_SHADER_PGM_LO_ES (the uploaded vertex code), and the GS user-data
     * dword that carries the compiler's ngg_lds_layout. */
    n = snprintf(buffer + used, sizeof(buffer) - used, "\nopenagc-ngg:");
    if (n < 0 || (size_t)n >= sizeof(buffer) - used) {
        return -1;
    }
    ...
    n = snprintf(buffer + used, sizeof(buffer) - used,
                 "\nopenagc-ngg-fixture: lds_layout=%08x slot=%u\n",
                 OPENAGC_NGG_USER_DATA_LAYOUT, OPENAGC_NGG_USER_DATA_LAYOUT_SLOT);
    if (n < 0 || (size_t)n >= sizeof(buffer) - used) {
        return -1;
    }
    used += (size_t)n;
...
int main(void)
{
    ...
    uint64_t baseline_va, probe_va, ngg_probe_va, table_va;
    ...
    if (sceKernelAllocateMainDirectMemory(OPENAGC_ARENA, OPENAGC_ARENA, 1,
                                          &physical) != 0) {
        return openagc_logf("openagc-draw-ngg: allocate failed\n") == 0 ? 0 : 1;
    }
    if (sceKernelMapNamedDirectMemory((void **)&arena, OPENAGC_ARENA,
                                      ...
                                      OPENAGC_ARENA, "openagc-draw-ngg") != 0) {
        return openagc_logf("openagc-draw-ngg: map failed\n") == 0 ? 0 : 1;
    }
    ...
    table = (uint32_t *)(arena + OPENAGC_TABLE_OFF);
    ...
    *marker = 0u;
    ...
    if ((color_va & 0xffull) != 0ull || (vert_code_va & 0xffull) != 0ull ||
        ...
        return openagc_logf("openagc-draw-ngg: VA not 256B aligned\n") == 0 ? 0
                                                                           : 1;
    }
    ...
    /* smoke.tri.vert covers the viewport with one triangle: three vertices,
     * rasterized as a strip (VGT_GS_OUT_PRIM_TYPE 2, the value Sony's own NGG
     * triangle draw carries), input topology DI_PT_TRILIST. */
    ...
    word_count = openagc_pm4_encode_draw_ngg_eop(
        &state, &program, baseline_va, probe_va, ngg_probe_va, vert_code_va,
        frag_code_va, table, table_va, OPENAGC_EOP_SEQUENCE, marker_va, words);
    if (word_count == 0u || word_count > OPENAGC_PM4_NGG_DRAW_EOP_WORDS) {
        return openagc_logf("openagc-draw-ngg: encode refused\n") == 0 ? 0 : 1;
    }
    ...
    if (gc_fd < 0) {
        return openagc_logf("openagc-draw-ngg: /dev/gc unavailable\n") == 0 ? 0
                                                                           : 1;
    }
    if (ioctl(gc_fd, OPENAGC_CONTEXT_QUERY, &submit) != 0) {
        close(gc_fd);
        return openagc_logf("openagc-draw-ngg: context query refused\n") == 0
                   ? 0
                   : 1;
    }
    ...
    if (ioctl(gc_fd, OPENAGC_SUBMIT_16, &submit) != 0) {
        close(gc_fd);
        return openagc_logf("openagc-draw-ngg: submit refused\n") == 0 ? 0 : 1;
    }
    ...
    if (openagc_pm4_draw_point_scan((const uint32_t *)(const void *)color,
                                    ...
                                    &value) == 0u) {
        return openagc_logf("openagc-draw-ngg: scan refused\n") == 0 ? 0 : 1;
    }
    ...
    if (openagc_write_draw_dump(completed, color_va, pixels, outside, guard,
                                value, match, baseline, probe, ngg,
                                window) != 0) {
        return 1;
    }
    return match ? 0 : 1;
