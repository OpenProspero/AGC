...
/* Step Y: SET_CONTEXT smoke-owned RT pairs + absolute COPY_DATA readback + EOP.
 *
 * Distinct from Step X (read-only absolute CB probe). Writes six non-zero
 * smoke.frag context values (SPI_SHADER_COL_FORMAT … CB_SHADER_MASK) via
 * proven SET_CONTEXT_REG, then reads them back with the Step-X proven
...
static const char openagc_log_path[] =
    "/data/prosperoai/openagc-ib-dump-ctxreg-rt.log";
...
int main(void)
{
    ...
    uint32_t words[OPENAGC_PM4_CTXREG_RT_EOP_WORDS];
    uint32_t word_count;
    uint32_t i;
    int gc_fd;
    int completed = 0;
    ...
    if (sceKernelAllocateMainDirectMemory(OPENAGC_ARENA, OPENAGC_ARENA, 1,
                                          &physical) != 0) {
        return openagc_logf("openagc-ctxreg-rt: allocate failed\n") == 0 ? 0 : 1;
    }
    if (sceKernelMapNamedDirectMemory((void **)&arena, OPENAGC_ARENA,
                                      ...
                                      OPENAGC_ARENA, "openagc-ctxreg-rt") != 0) {
        return openagc_logf("openagc-ctxreg-rt: map failed\n") == 0 ? 0 : 1;
    }
    ...
    *marker = 0u;
    ...
    word_count = openagc_pm4_encode_ctxreg_rt_abs_eop(
        dest_va, OPENAGC_EOP_SEQUENCE, marker_va, words);
    if (word_count != OPENAGC_PM4_CTXREG_RT_EOP_WORDS) {
        return openagc_logf("openagc-ctxreg-rt: encode size mismatch\n") == 0 ? 0
                                                                              : 1;
    }
    memcpy(ib, words, sizeof(words));

    cb[0].header = ((uint64_t)ib_va << 32) | 0xC0023F00u;
    cb[0].ib_base = ((uint64_t)OPENAGC_PM4_CTXREG_RT_EOP_WORDS << 32) |
                    ((uint64_t)ib_va >> 32);

    gc_fd = open("/dev/gc", O_RDWR);
    if (gc_fd < 0) {
        return openagc_logf("openagc-ctxreg-rt: /dev/gc unavailable\n") == 0 ? 0
                                                                             : 1;
    }
    if (ioctl(gc_fd, OPENAGC_CONTEXT_QUERY, &submit) != 0) {
        close(gc_fd);
        return openagc_logf("openagc-ctxreg-rt: context query refused\n") == 0
                   ? 0
                   : 1;
    }
    ...
    if (ioctl(gc_fd, OPENAGC_SUBMIT_16, &submit) != 0) {
        close(gc_fd);
        return openagc_logf("openagc-ctxreg-rt: submit refused\n") == 0 ? 0 : 1;
    }
    ...
    if (openagc_write_ctxreg_dump(completed, dest, OPENAGC_GFX10_CTXREG_RT_COUNT) !=
        0) {
        return 1;
    }
