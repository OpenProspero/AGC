...
#define OPENAGC_SPAN2_LANES 16u
...
static const char openagc_log_path[] = "/data/prosperoai/openagc-store-span2.log";
...
int main(void)
{
    ...
    uint32_t words[OPENAGC_PM4_COMPUTE_STORE_SPAN2_WORDS];
    uint32_t i;
    int gc_fd;
    int completed = 0;
    int matched = 1;
    ...
    if (sceKernelAllocateMainDirectMemory(OPENAGC_ARENA, OPENAGC_ARENA, 1,
                                          &physical) != 0) {
        return openagc_logf("openagc-store-span2: allocate failed\n") == 0 ? 0 : 1;
    }
    if (sceKernelMapNamedDirectMemory((void **)&arena, OPENAGC_ARENA,
                                      ...
                                      OPENAGC_ARENA, "openagc-sp2") != 0) {
        return openagc_logf("openagc-store-span2: map failed\n") == 0 ? 0 : 1;
    }
    ...
    if ((code_va & 0xffu) != 0u) {
        return openagc_logf("openagc-store-span2: code VA misaligned\n") == 0 ? 0 : 1;
    }
    ...
    *marker = 0u;
    ...
    openagc_pm4_encode_compute_store_span2(code_va, destination_va, OPENAGC_EOP_SEQUENCE,
                                           marker_va, words);
    memcpy(ib, words, sizeof(words));

    cb[0].header = ((uint64_t)ib_va << 32) | 0xC0023F00u;
    cb[0].ib_base = ((uint64_t)OPENAGC_PM4_COMPUTE_STORE_SPAN2_WORDS << 32) |
                    ((uint64_t)ib_va >> 32);

    gc_fd = open("/dev/gc", O_RDWR);
    if (gc_fd < 0) {
        return openagc_logf("openagc-store-span2: /dev/gc unavailable\n") == 0 ? 0 : 1;
    }
    if (ioctl(gc_fd, OPENAGC_CONTEXT_QUERY, &submit) != 0) {
        close(gc_fd);
        return openagc_logf("openagc-store-span2: context query refused\n") == 0 ? 0 : 1;
    }
    ...
    if (ioctl(gc_fd, OPENAGC_SUBMIT_16, &submit) != 0) {
        close(gc_fd);
        return openagc_logf("openagc-store-span2: submit refused\n") == 0 ? 0 : 1;
    }
    ...
    for (i = 0u; i < OPENAGC_SPAN2_LANES; ++i) {
        if (destination[i] != OPENAGC_STORE_SPAN_VALUE) {
            matched = 0;
        }
    }
    if (destination[OPENAGC_SPAN2_LANES] != 0xCCCCCCCCu) {
        matched = 0;
    }

    (void)openagc_logf(
        "openagc-store-span2: submit=ok completed=%d matched=%d lanes=%u "
        "head=%08x mid=%08x tail=%08x beyond=%08x marker=%llx\n",
        completed, matched, OPENAGC_SPAN2_LANES, destination[0], destination[8],
        destination[OPENAGC_SPAN2_LANES - 1u], destination[OPENAGC_SPAN2_LANES],
