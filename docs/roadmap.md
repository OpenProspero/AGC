...
2. **Draw/render PM4**: real render-target and draw packets have no
   ...
   submitted. Step AA added a read-only GB tile-mode table vehicle
   (`tag=mmio-tilemode`): absolute `COPY_DATA` of `GB_ADDR_CONFIG` +
   `GB_TILE_MODE0..31` with a fail-closed
   (`ARRAY_MODE`, `MICRO_TILE_MODE_NEW`) index lookup. Console-negative
   (`completed=0`, poison): the `0x13xx` GB register block is not
